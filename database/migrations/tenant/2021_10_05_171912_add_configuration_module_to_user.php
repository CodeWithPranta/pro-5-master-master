<?php
    ...
    class AddConfigurationModuleToUser extends Migration
    {
        ...
        public function up()
        {
            ...
            $s = [];
            if (!empty($e)) {
                $s[] = $this->DbModuleLevel($e->id, 'configuration_company', "Empresa", $admin_id, true,76);
                $s[] = $this->DbModuleLevel($e->id, 'configuration_advance', "Avanzado", $admin_id, true,77);
                $s[] = $this->DbModuleLevel($e->id, 'configuration_visual', "Visual", $admin_id, true,78);
                $this->DbModuleUser($admin_id, $e->id,true);
            }

        }

        public function DbModuleLevel($config_id, $module_name, $description, $user_id, $create = true,$id)
        {
            $toSearch = [
                'module_id' => $config_id,
                'value' => $module_name,
        ...
        public function down()
        {
            ...
            $s = [];
            if (!empty($e)) {
                $s[] = $this->DbModuleLevel($e->id, 'configuration_company', "Empresa", $admin_id, false,76);
                $s[] = $this->DbModuleLevel($e->id, 'configuration_advance', "Avanzado", $admin_id, false,77);
                $s[] = $this->DbModuleLevel($e->id, 'configuration_visual', "Visual", $admin_id, false,78);
